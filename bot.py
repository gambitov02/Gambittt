...
def set_subscribed(user_id: int, subscribed: bool) -> None:
    now = datetime.utcnow().isoformat()
    with db_connect() as conn:
        conn.execute(
            """
            ...
            """,
            (user_id, 1 if subscribed else 0, now, now),
        )
        conn.commit()


def get_subscribed(user_id: int) -> bool:
    with db_connect() as conn:
        row = conn.execute(
...
def get_subscribers() -> Iterable[int]:
    with db_connect() as conn:
...
def save_last_payment(user_id: int, payment_id: str) -> None:
    now = datetime.utcnow().isoformat()
    with db_connect() as conn:
        conn.execute(
            """
            ...
            ON CONFLICT(user_id) DO UPDATE SET
                ...
                updated_at=excluded.updated_at
            """,
            (user_id, payment_id, now),
        )
        conn.commit()


def get_last_payment(user_id: int) -> Optional[str]:
    with db_connect() as conn:
        row = conn.execute("SELECT payment_id FROM payments WHERE user_id=?", (user_id,)).fetchone()
...
async def cmd_menu(message: Message):
    user_id = message.from_user.id
    upsert_user(user_id)
    await message.answer("Меню 👇", reply_markup=menu_kb(get_subscribed(user_id)).as_markup())
...
async def cb_toggle_sub(call: CallbackQuery):
    ...
    current = get_subscribed(user_id)
    set_subscribed(user_id, not current)
...
async def cb_pay(call: CallbackQuery, bot: Bot):
    ...
    save_last_payment(user_id, payment_id)
    ...
    await call.message.answer(
        ...
        reply_markup=menu_kb(get_subscribed(user_id)).as_markup(),
    )
...
async def cb_check(call: CallbackQuery):
    ...
    payment_id = get_last_payment(user_id)
    if not payment_id:
...
async def cb_access(call: CallbackQuery, bot: Bot):
    ...
    # Защита: платёж должен быть для этого пользователя
